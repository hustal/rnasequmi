...
========================================================================================
                         nf-core/rnasequmi
========================================================================================
 nf-core/rnasequmi Analysis Pipeline.
 #### Homepage / Documentation
 https://github.com/nf-core/rnasequmi
----------------------------------------------------------------------------------------
*/
...
def create_workflow_summary(summary) {
    ...
    yaml_file.text  = """
    id: 'nf-core-rnasequmi-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/rnasequmi Workflow Summary'
...
workflow.onComplete {
    ...
    def subject = "[nf-core/rnasequmi] Successful: $workflow.runName"
    if (!workflow.success) {
      subject = "[nf-core/rnasequmi] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/rnasequmi] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/rnasequmi] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
          ...
          log.info "[nf-core/rnasequmi] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
          // Catch failures and try with plaintext
          [ 'mail', '-s', subject, email_address ].execute() << email_txt
          log.info "[nf-core/rnasequmi] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "${c_purple}[nf-core/rnasequmi]${c_green} Pipeline completed successfully${c_reset}"
    } else {
        checkHostname()
        log.info "${c_purple}[nf-core/rnasequmi]${c_red} Pipeline completed with errors${c_reset}"
    }
...
def nfcoreHeader(){
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/rnasequmi v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
